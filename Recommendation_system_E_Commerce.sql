...
-- 8. Update product price for a specific product:
UPDATE Products SET price = 1500 WHERE product_id = 'P001';

-- 9. Retrieve the top 5 users with the highest total purchase amount:
SELECT user_id, SUM(total_amount) AS total_purchase_amount 
FROM Past_Purchases 
GROUP BY user_id 
...
-- 10. Count the number of unique brands in the Products table:
SELECT COUNT(DISTINCT brand) AS unique_brands FROM Products;

-- 11. Window function to rank products by price within each category:
SELECT product_id, product_name, category, price, 
       RANK() OVER (PARTITION BY category ORDER BY price) AS price_rank
FROM Products;

-- 12. Common Table Expression (CTE) to find the average price of products:
WITH AvgPrice AS (
    SELECT AVG(price) AS average_price FROM Products
)
SELECT * FROM Products WHERE price > (SELECT average_price FROM AvgPrice);

-- 13. Retrieve the product with the highest total purchase amount:
...
-- 14. Count the number of interactions per product
SELECT product_id, COUNT(*) AS interaction_count
FROM Interactions
GROUP BY product_id;

-- 15. List top N most popular products based on interactions
SELECT product_id, COUNT(*) AS interaction_count
FROM Interactions
GROUP BY product_id
...
-- 16. Retrieve product details along with user interactions:
SELECT p.*, i.interaction_type, i.user_id
FROM Products p
INNER JOIN Interactions i ON p.product_id = i.product_id;

-- 17. Find products with no interactions:
SELECT p.*
FROM Products p
LEFT JOIN Interactions i ON p.product_id = i.product_id
WHERE i.product_id IS NULL;

-- 18. Rank products by price within each category using window functions:
SELECT product_id, product_name, category, price,
       RANK() OVER (PARTITION BY category ORDER BY price) AS price_rank
FROM Products;

-- 19. Calculate the cumulative sum of total purchases by user:
SELECT user_id, purchase_id, total_amount,
       SUM(total_amount) OVER (PARTITION BY user_id ORDER BY purchase_id) AS cumulative_sum
FROM Past_Purchases;

-- 20. Retrieve interactions for products with prices above the average price:
SELECT *
FROM Interactions
WHERE product_id IN (
    SELECT product_id
...
-- 21. Create a CTE to calculate average product price by category:
...
-- 22. Use a CTE to find the top 3 users with the highest total purchase amounts:
...
-- 23. Calculate the percentage contribution of each product to the total sales amount:
...
-- 24. Identify users who made purchases of more than $500 in a single transaction:
SELECT user_id, purchase_id, total_amount
FROM Past_Purchases
WHERE total_amount > 500;

-- 25. Find users who made purchases in the first and last quarter of the year:
SELECT user_id
FROM Past_Purchases
WHERE EXTRACT(QUARTER FROM purchase_date) = 1
   OR EXTRACT(QUARTER FROM purchase_date) = 4;

-- 26. Calculate the average quantity of products purchased by users who interacted with products priced above the average price:
...
-- 27. Find users who have interacted with products across multiple categories:
SELECT user_id
FROM (
